...
class CandlestickSeriesWithAnnotation extends Component {
	constructor(props) {
		super(props);
...
CandlestickSeriesWithAnnotation.propTypes = {
...
CandlestickSeriesWithAnnotation.defaultProps = {
...
function drawOnCanvas(ctx, props, moreProps) {
	...
	wickNest.forEach(outer => {
		...
		values.forEach(each => {
			...
			console.log(each.annotation);
			if (each.annotation) {
				canvas_arrow(ctx, d.x - 0.5, d.y1 - 25, d.x - 0.5, d.y1 - 5);
				ctx.font = ctx.font.replace(/\d+px/, "14px");
				ctx.fillText(each.annotation, d.x - 10, d.y1 - 30);
			}
		});
	});
...
function getAnnotation(d) {
	if (d.date.setHours(0, 0, 0, 0) == new Date("2014-4-24").getTime()) {
		return "BUY";
	}
	if (d.date.setHours(0, 0, 0, 0) == new Date("2015-2-23").getTime()) {
		return "SELL";
	}
	return "";
}
...
export default CandlestickSeriesWithAnnotation;
